...
- `room`: _(string)_ The island within that realm that the player is connected to
...
> Note: When the scene first loads, there might not yet be an island assigned for them. The explorer will always eventually assign an island to the player, but this can sometimes occur a couple of seconds after the scene is loaded.
