...
## Fetch more player info

You can make a request to the following API to obtain info about the player from the content server. Besides including the player name, and wallet (which you can obtain through `userData`) you can also find a full list of all the wearables that the player owns and all the wearables that the player is currently wearing.

This feature could be used, for example, to only allow players to enter a place if they are wearing a special wearable item.

This information is exposed in the following URL, appending the player's user id to the url parameter.

`https://peer.decentraland.org/content/entities/profiles?pointer=<player user id>`

It can be used in a scene together with `getUserData()` like this:
...
fetchWearablesData() {
    let url = `https://peer.decentraland.org/content/entities/profiles?pointer={userData.userId}`.toString()
	log('using URL: ', url)
    ...
    executeTask(async () => {
      try {
		...
		log("an error occured while reaching for player data")
	  }
	})
```
