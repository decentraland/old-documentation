...
> Tip: For text that's meant to float in space, set the entity's Transform component to have a Billboard mode so that the text rotates to always face the user and be legible. See [Position entities]({{ site.baseurl }}{% post_url /development-guide/2018-02-5-set-entity-position %}).
