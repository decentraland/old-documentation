...
```ts
myText.get(TextShape).value = "new string"
```
...
> Tip: It a text is meant to float in space, it's a good idea to add a [`Billboard` component]({{ site.baseurl }}{% post_url /development-guide/2018-01-12-entity-positioning %}#face-the-user) so that the text rotates to always face the user and be legible.
...
```ts
myText.add(new TextShape("Text"))
myText.get(TextShape).isPickable = false
```
