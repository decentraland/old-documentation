...
When testing in preview, be sure to run the scene with `dcl start --web3` so that you connect with your wallet. Otherwise, won't there won't be any information to retrieve on the catalyst servers about the random guest account generated by the preview.
